...
volatile unsigned char buffer[MAX_QSIZE]={0};   // UART 부터 읽어온 data를 \r 이나 \n을 만날때 까지 저장할 공간
volatile int rxReadyFlag=0;   // \r이나 \n을 만났다는 인디케이터 flag변수
//-------  Circular queue  start -----------
#if 1  // Circular queue
volatile int front = -1;      // output pointer
volatile int rear  = -1;      // input pointer

int IsEmpty(void)
{
	if (front == rear)		//front와 rear가 같으면 큐는 비어있는 상태
		return TRUE;		// 참(1)을 리턴한다.
	else FALSE;             // 저짓을 리턴한다.
}

int IsFull(void)
{
	int tmp=(rear+1)% MAX_QSIZE; //원형 큐에서 rear+1을 MAX로 나눈 나머지값이
...
void inputq(unsigned char value)
{
	if (IsFull())
